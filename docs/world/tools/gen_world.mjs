...
// Usage (repo root):  node docs/world/tools/gen_world.mjs [--quick] [--no-render]
// Outputs: docs/world/heightmap.png (16-bit), heightmap.json, tools/out/carto.json (measured data), [topo.png + tools/out/*.png unless --no-render]
...
for (let li = 0; li < S.LAKES.length; li++) {
  ...
  let leak = 0;
  const DAM = L.dam || 4, QMAX = DAM > 4 ? 1.5 + Math.min(0.6, (DAM - 4) * 0.05) : 1.5;
  forBox(L.x - L.rx * (QMAX + 0.15), L.z - L.rz * (QMAX + 0.15), L.x + L.rx * (QMAX + 0.15), L.z + L.rz * (QMAX + 0.15), (i, j, x, z, k) => {
    ...
    if (q < 1) {
      ...
      // rim (natural levee / moraine).  L.dam (m, default 4) = max raise; dams wider than 4 m get a wider, gentler skirt.
      const rim = L.level + 0.3 + 1.2 * smoothstep(1, 1.5, q);
      const need = rim - H[k];
      if (need > 0) { const add = Math.min(need, DAM) * (1 - smoothstep(1.2, QMAX, q)); H[k] += add; if (need > DAM && q < 1.1) leak++; }
    }
  });
  L._leakCells = leak;
}
...
fs.writeFileSync(path.join(OUT, 'heightmap.png'), encodeGray16(N, N, U16, { Title: 'Brumeval heightmap v0.3', Comment: 'metres = value*0.01 - 60 ; sample (i,j) at x=-2304+4.5i, z=-2808+4.5j' }));
const meta = {
  ...
  generator: 'docs/world/tools/gen_world.mjs (deterministic, seed ' + SEED + ')',
};
...
fs.writeFileSync(path.join(OUT_TOOLS, 'carto.json'), JSON.stringify(carto));
