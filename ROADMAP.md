...
- Pierres de téléportation (voyage rapide) + **carte du monde ouvrable depuis le menu (bouton « Carte ») et avec la touche M** : position et direction du joueur, régions, villes, pierres de téléportation, donjons, marqueurs de quête, zones rouges, brouillard de guerre à découvrir, zoom et déplacement, repère personnel posé par clic. Une première version pour la carte actuelle arrive dans une petite mise à jour juste après la v0.2 (v0.2.1).
...
- Au niveau 1, chaque classe n'a **que son attaque de base**. Tout le monde apprend d'abord les **Fondamentaux** au cœur de l'arbre : **Roulade, Sprint, Saut** (nouveau : petit saut qui passe au-dessus des ondes de choc au sol et permet des attaques sautées), **Garde** (blocage, puis parade parfaite en variante) et **Attaque chargée**. La région de sa classe s'ouvre après 3 Fondamentaux, puis on se spécialise. Les personnages existants gardent la roulade et le sprint qu'ils avaient déjà. Chaque niveau donne **1 point** (plus 1 point bonus tous les 5 niveaux), à placer où l'on veut.
- **Un seul très grand arbre commun, « l'Arbre des Brumes »** : au centre, la branche **Survie** (endurance, roulade, déplacements) ; autour, les régions **Guerrier**, **Mage** et **Rôdeur** (3 branches chacune) ; entre elles, des **passerelles hybrides** (Lame spirituelle, Arcaniste sylvestre, Chasseur). Chaque personnage démarre dans la région de sa classe, **mais peut aller n'importe où** : plus besoin de créer un autre personnage pour changer de style (on garde quand même plusieurs personnages par compte).
