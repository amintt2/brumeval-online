...
for key in KEYS:
    ...
    for level in range(3):
        ...
        for mesh in doc['meshes']:
            for primitive in mesh['primitives']:
                assert all(semantic in primitive['attributes'] for semantic in ('POSITION','NORMAL','TEXCOORD_0','TANGENT')),(path,primitive['attributes'])
    ...
    for suffix in ['', '_turntable','_wire','_normals','_uv','_lod1','_lod2']:
...
bow_labels=['Arc court — bois sombre','Arc long — bois cuivré','Arc elfique — bois clair et or','Arc royal — or et cuir sombre']
bows=Image.new('RGB',(1200,336),'#202123');bow_draw=ImageDraw.Draw(bows)
font_path=Path('C:/Windows/Fonts/segoeui.ttf')
bow_font=ImageFont.truetype(str(font_path),15) if font_path.exists() else ImageFont.load_default(size=15)
for i,label in enumerate(bow_labels,1):
    with Image.open(pre/f'eq_bow_{i}.png') as image:bows.paste(image.convert('RGB').resize((300,300)),((i-1)*300,0))
    bow_draw.text(((i-1)*300+9,308),label,fill='#e1cf9d',font=bow_font)
bows.save(pre/'bows-current.jpg',quality=95)
before=pre/'bow-revision-before'
if before.exists():
    comparison=Image.new('RGB',(1200,700),'#202123');cd=ImageDraw.Draw(comparison)
    cd.text((10,8),'AVANT',fill='#e1cf9d')
    for i in range(1,5):
        with Image.open(before/f'eq_bow_{i}.png') as image:comparison.paste(image.convert('RGB').resize((300,300)),((i-1)*300,30))
    cd.text((10,344),'APRES',fill='#e1cf9d');comparison.paste(bows,(0,364))
    comparison.save(pre/'bow-comparison.jpg',quality=94)
cards=''.join(f'<article><h2>{r["key"]}</h2><img src="{r["key"]}.png"><p>{r["variants"][0]["triangles"]} triangles · {r["variants"][0]["bytes"]//1024} Ko</p><p>'+ ' · '.join(f'<a href="{r["key"]}_{s}.png">{label}</a>' for s,label in [('turntable','8 angles'),('wire','Maillage'),('normals','Normales'),('uv','UV'),('lod1','LOD1'),('lod2','LOD2')])+'</p></article>' for r in records)
(pre/'review.html').write_text('<!doctype html><html lang="fr"><meta charset="utf-8"><title>Brumeval — équipements</title><style>body{background:#17191b;color:#d9d1bd;font:16px system-ui;max-width:1500px;margin:32px auto;padding:20px}main{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:24px}article{background:#25282b;padding:16px;border:1px solid #554b36}img{width:100%}h2{font-size:18px}a{color:#d6b578}</style><h1>18 équipements — revue CX-3</h1><p>Textures procédurales cuites, pivot à la prise en main, 3 niveaux de détail. Assets prêts à intégrer par Claude ; attache aux personnages à vérifier dans le jeu.</p><h2>Arcs : quatre identités visibles</h2><img src="bows-current.jpg"><p><a href="bow-comparison.jpg">Comparaison avant / après</a></p><main>'+cards+'</main></html>',encoding='utf-8')
print(f'PASS: {len(KEYS)*3} GLB, textures WebP embarquées, budgets, pivots, LOD et {len(KEYS)*7} images QA.')
