...
export const SCHEMAS = {
  register: { name: str(64), password: str(256), cls: str(32) },
  login: { name: str(64), password: str(256) },
...
export function checkStructure(value, depth = 0) {
  if (value === null) return null;
  const t = typeof value;
  if (t === 'string') return value.length > CHAT_MAX_LEN * 4 ? 'string_too_long' : null;
  if (t === 'number') return Number.isFinite(value) ? null : 'bad_number';
  if (t === 'boolean') return null;
  if (t !== 'object') return 'bad_type';
  if (depth >= MAX_DEPTH) return 'too_deep';
  if (Array.isArray(value)) {
    if (value.length > MAX_ARRAY) return 'array_too_long';
    for (const v of value) {
      const r = checkStructure(v, depth + 1);
      if (r) return r;
    }
    return null;
  }
  ...
  for (const k of keys) {
    ...
    const v = value[k];
    if (typeof v === 'string' && v.length > MAX_STRING && !(depth === 0 && k === 'text')) return 'string_too_long';
    const r = checkStructure(v, depth + 1);
    if (r) return r;
  }
  return null;
