...
test('permessage-deflate is negotiated; batch frames only for clients that ask; auth_ok carries ver/build', async () => {
  await withServer(async (srv) => {
    ...
    plain.send(JSON.stringify({ t: 'register', name: 'Plat', password: 'abcdef', cls: 'mage' }));
    batched.send(JSON.stringify({ t: 'register', name: 'Groupe', password: 'abcdef', cls: 'warrior' }));
    ...
    for (let i = 0; i < 200; i++) batched.send(JSON.stringify({ t: 'ping', c: i }));
